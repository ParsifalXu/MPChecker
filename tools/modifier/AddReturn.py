...
class AddReturn(ast.NodeTransformer):
    ...
    def visit_Return(self, node):
        ...
        return_value = " ^ ".join([f"({arg} = {{{arg}}})" for arg in args])
        if isinstance(node.value, ast.Constant) and not isinstance(node.value.value, int) and ("ERROR" in node.value.value or "WARNING" in node.value.value or "ASSERT" in node.value.value):
            return node
        return_expr = ast.parse(f'f"{return_value}"').body[0].value
